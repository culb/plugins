...
sub conv
{
    ...
    +{ 
        time => sub# MUST be seconds passed to this function.
        {
...
hook_command( 'CNS', sub
{
        LABEL:
        my $one = hook_timer( 0, sub
        {
            conv->{nspeed}();
            return REMOVE;
        });
        
        my $two = hook_timer( 1000, sub
        {
            conv->{nspeed}();
            return REMOVE;
        });
...
register(
    'culb Windows sysinfo',
    0x02,
    'System information. Annoy people with your computers information.',
    sub{ command('MENU DEL Sysinfo'); }
);
